...
def makeDCfunc(raw_z_data):
    ...
    if (bundled_count > 0):
        ...
        for z in z_bundles:
            ...
            raw_super_z_data[z].extend(uncommon_c)
            raw_super_z_data[z].extend([f"s{bundle[0]}" for bundle in z_bundles[z]])
    ...
    print("Total LE solution cost:", finalLEcost,
          "\nCost without recursion:", totalLEn(raw_z_data, z_bundles, bundled_LEn),
          "\nOriginal cost:", plainLEn(raw_z_data), "\n")
    
    return finalLEcost


makeDCfunc(raw_z_data)